...
所以做后端服务的时候，为了提升查询效率，我们不会只用 mysql，一般会结合**内存数据**库来做缓存，最常用的是 **Redis**
...
![image-20250205115653374](./assets/image-20250205115653374.png)

可以直接点击`run`，下载并开启一个容器

![image-20250205120051490](./assets/image-20250205120051490.png)

redis 服务跑起来之后，我们可以先用docker容器中的redis-cli 操作一下。

![image-20250205120855300](./assets/image-20250205120855300.png)

## RedisInsight

如果觉得命令行不够清晰，也能下载Redis给我提供了GUI，[官网下载](https://redis.io/insight/#insight-form)即可，不过下载之前需要稍微填写一下信息

![image-20250205160506786](./assets/image-20250205160506786.png)

安装成功之后，就会自动搜索到当前已经启动的Redis服务，点击这个数据库，就可以展示我们前面创建的各种类型的key和value

![image-20250205160714647](./assets/image-20250205160714647.png)

现在可以通过工作，简单的进行操作即可

![image-20250205161340213](./assets/image-20250205161340213.png)

当然你要写cli命令也可以

![image-20250205161658662](./assets/image-20250205161658662.png)
...
![image-20250205151409422](./assets/image-20250205151409422.png)

### 2、列表操作

在Redis中，列表通常应用于**消息队列、任务队列、时间线等**场景，操作其实类似于JS中的数组操作。

列表操作的常用命令如下：

- LPUSH key value：将值推入列表左侧
- RPUSH key value：将值推入列表右侧
- LPOP key value：从列表左侧删除值（value标识删除个数）
- RPOP key value：从列表右侧删除值
- LLEN key：获取列表长度
- LRANGE key start stop：获取列表指定范围的值
  > 0开始；-1表示末尾

![image-20250205153406950](./assets/image-20250205153406950.png)

![image-20250205153538508](./assets/image-20250205153538508.png)

### 3、集合操作

在Redis中，集合由一组**无序但唯一**的成员组成。使用集合可以对数据执行交集、并集、差集等操作。
...
- SREM key members：移除集合中的成员
- SMEMBERS key：获取集合的所有成员
- SINTER key1 key2：获取多个集合的交集

![image-20250205154804224](./assets/image-20250205154804224.png)

### 4、有序集合操作

与集合不同，Sorted Set（有序集合，也被称为ZSet）是由一**组按照分数排序并且唯一的数据组成**的，如果要用于比如排行榜的处理，就可以使用这种数据结构
...
![image-20250205162005347](./assets/image-20250205162005347.png)

![image-20250205162043135](./assets/image-20250205162043135.png)

通过命令操作一下

![image-20250205162324433](./assets/image-20250205162324433.png)

![image-20250205162354691](./assets/image-20250205162354691.png)
...
哈希结构适用于存**储复杂对象结构的数据**，例如用户信息，订单信息等等，**每个Key代表不同的对象**，field value代表对象的属性和值。比如要存储`user`信息，执行如下的命令设置字段：
...
![image-20250205162714414](./assets/image-20250205162714414.png)

![image-20250205162819948](./assets/image-20250205162819948.png)
...
![image-20250205164515099](./assets/image-20250205164515099.png)

![image-20250205164808454](./assets/image-20250205164808454.png)

比如获取bike1和bike2的坐标信息

![image-20250205164932784](./assets/image-20250205164932784.png)
...
![image-20250205165359124](./assets/image-20250205165359124.png)
...
在应用中，如果我们**希望记录用户的活跃状态、在线状态、访问频率等等**，就可以使用位图。
...
![image-20250205171521094](./assets/image-20250205171521094.png)

![image-20250205171607303](./assets/image-20250205171607303.png)
...
![image-20250205171840760](./assets/image-20250205171840760.png)

或者统计用户总共有多少次访问记录

![image-20250205171953853](./assets/image-20250205171953853.png)
