...
![image-20250127010710155](./assets/image-20250127010710155.png)

除了在拦截器上直接指定文件上传后的保存位置之外，`Multer`中间件提供了文件存储引擎，我们可以在`app.module.ts`中通过动态模块进行配置
不过稍微注意的是，**默认不会自动帮你创建文件夹**。
...
![image-20250131121748637](./assets/image-20250131121748637.png)
...
![image-20250131122019629](./assets/image-20250131122019629.png)
...
![image-20250131123253953](./assets/image-20250131123253953.png)
...
要对上传的文件做一些限制，比如文件大小、类型等，这部分就可以利用我们讲过的 `pipe` 处理(例如： `ParseFilePipe`)，比如我们完全可以自己创建一个Pipe
...
![image-20250131130624645](./assets/image-20250131130624645.png)
