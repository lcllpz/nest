...
CREATE TABLE user_detail(
	...
	-- 外键：user_detail.user_id 的值必须在 user.id 里存在
	-- CONSTRAINT user_detail_person 不是多出来的业务逻辑，只是给外键约束贴标签，方便以后用 ALTER TABLE ... DROP FOREIGN KEY 名字 管理。真正起关联作用的是后面的 FOREIGN KEY ... REFERENCES ...。
)
```
...
![image-20250201135235705](./assets/image-20250201135235705.png)

![image-20250201135254824](./assets/image-20250201135254824.png)
...
![image-20250201135532618](./assets/image-20250201135532618.png)
...
![image-20250201140625105](./assets/image-20250201140625105.png)

![image-20250201140647014](./assets/image-20250201140647014.png)
...
![image-20250201140849967](./assets/image-20250201140849967.png)
...
![image-20250201141021823](./assets/image-20250201141021823.png)
...
CREATE TABLE tag_article(
	...
	-- ON DELETE CASCADE：删标签或删文章时，中间表里相关行自动删掉，不会留下「悬空」的 tag_id / article_id
	-- ON UPDATE CASCADE 若父表主键 id 被修改（很少见），子表外键跟着改
	FOREIGN KEY(article_id) REFERENCES article(id) ON DELETE CASCADE ON UPDATE CASCADE,
	UNIQUE(tag_id,article_id)
	-- 联合唯一：同一篇文章、同一个标签，只能关联一次。
)
...
![image-20250201142259035](./assets/image-20250201142259035.png)

![image-20250201142318123](./assets/image-20250201142318123.png)

![image-20250201142340703](./assets/image-20250201142340703.png)
...
![image-20250201142940776](./assets/image-20250201142940776.png)
