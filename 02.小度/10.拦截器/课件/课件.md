...
拦截器不同于中间件和守卫，它在路由请求之前和之后都可以进行逻辑处理，能够充分操作`request`和`response`对象。拦截器通常用于`记录请求日志、转换或者格式化相应数据等等`
...
`intercept`方法返回`Observable（可观察对象）`对象，而`Observable`对象位于`RxJS`库中

### RxJS

**[RxJS](https://rxjs.dev/)**是一个用于**处理异步数据流的JavaScript库**。你可以把它理解为一个管道，它可以帮你更方便的处理各种事件和数据流，当然我们没必要增加心智负担，可以更直白的认为它就是和前端的`Lodash`工具库差不多的一个库。

> pipe 就是把多个操作符按顺序串成一条「流水线」，数据从一头进，经过每一道工序，再从另一头出来。

> Promise：发起 → 等待 → 一次 resolve/reject
> Observable：可以发出 0 次、1 次或多次 值，还能被取消、组合、变换、串行。 订阅 → 0~N 次 next → complete 或 error（可 unsubscribe 取消）

> RxJS 用来**用「流 + 操作符」的方式组织异步和多步数据处理**；`Nest 拦截器返回 Observable，就是为了在请求进入控制器之后、响应出去之前，用 pipe 优雅地插入、变换、捕获错误`。

`Controller` 之前之后的处理逻辑**可能是异步的**，所以可以使用 `RxJS` 的各种 `operator`.它是由数据源产生数据，经过一系列 operator 的处理，最后传给接收者。这个数据源叫做 `observable`。

**RxJS 的核心思想:**
通过创建和订阅 `Observable`，使得你能够以同步的方式处理原本异步的事件流。这些异步操作通过 `Observable` 的内建机制被包装，并且可以利用 RxJS 提供的操作符进行组合、转换和调度。
