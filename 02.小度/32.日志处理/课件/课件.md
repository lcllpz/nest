...
![image-20250213144100760](./assets/image-20250213144100760.png)
...
![image-20250213145139058](./assets/image-20250213145139058.png)
...
`bufferLogs`表示先不打印日志，把它放到buffer缓冲区，直到用`useLogger`指定了Logger并且应用初始化完毕，在 Logger 输出
...
![image-20250213165918615](./assets/image-20250213165918615.png)
...
![image-20250213161818359](./assets/image-20250213161818359.png)

当访问getHello路由，打印如下：

![image-20250213161905032](./assets/image-20250213161905032.png)
...
![image-20250213170116566](./assets/image-20250213170116566.png)

同时，当我们访问路由之后：

![image-20250213170249894](./assets/image-20250213170249894.png)

至此，我们已经完成日志本地持久化的功能。不过在实际业务中，我们也可以将日志文件上传到远程服务器，或者通过指定transports.Http的方式将日志持久化到数据库中，以便进行后续的业务数据分析、数据过滤和数据清洗等操作。

```js
// 本地文件 = 本机备份与开发调试；远程上传或 Http = 生产里集中收集，方便后续检索、统计和治理。
// 具体 host/path 取决于你们用的日志服务；Winston 只负责把日志 POST 出去，是否进数据库由接收端决定
new transports.Http({
