...
在Nest中，`模块`通过@Module装饰器来声明。每个应用都会有一个根模块，Nest框架会从根模块开始收集各个模块之间的依赖关系，形成依赖关系树。在应用初始化时，根据依赖关系树实例化不同的模块对象。如图：

![image-20250124154209700](./assets/image-20250124154209700.png)
...
前面介绍的都是静态模块的绑定和使用。Nest还提供了动态加载模块的功能，使得应用可以在运行时创建模块，通常用于`动态读取配置或者根据权限判断来加载模块`。
...
- 解决的核心问题是：静态 @Module({...}) 在编译时就固定了，无法在 imports 时传入不同配置。

使用场景：

- 传入配置（最常见）
  > imports 静态条件注册适合 启动时就知道要不要加载；LazyModuleLoader 适合 运行过程中才决定加载 的场景。
