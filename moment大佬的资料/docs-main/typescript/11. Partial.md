在 TypeScript 项目开发中，或多或少会遇到一些类型与类型之间需要转换的情况。强大的 TypeScript 类型系统为开发者提供了部分常用的内置全局工具类型。

接下来的一系列文章我们将学习这些内置的全局工具类型，首先登场的是：Partial。

## 什么是 Partial

Partial 是 TypeScript 提供的一个实用工具类型（Utility Type），用于将对象类型中的所有属性变为可选属性。
...
![20240625172432](https://raw.githubusercontent.com/xun082/md/main/blogs.images20240625172432.png)
...
![20240625173158](https://raw.githubusercontent.com/xun082/md/main/blogs.images20240625173158.png)
...
在实际应用中，通过使用这些高级类型可以更好地优化代码质量，让你的代码更加清晰和易于维护。Partial 作为 TypeScript 内置工具类型的基础，掌握它的使用方法和实现原理，对于编写类型安全的代码非常重要。
